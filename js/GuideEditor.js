...
class GuideEditor {
	...
	overlayClick(e) {
		if (e.target === document.getElementById("ed-overlay")) this.close();
	}
	...
	togIcons(e) {
		...
		document.getElementById("em-icon-grid").innerHTML = GUIDE_ICONS.map((ico) => `<button onclick="app.editor.selectIcon('${ico}')">${ico}</button>`).join(
			"",
		);
