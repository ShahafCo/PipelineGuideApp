...
class GuideViewer {
	...
	render() {
		...
		const tags = (g.meta.tags || [])
			.map((t) => `<span class="tag ac" onclick="app.sidebar.selectTag('${t.replace(/'/g, "\\'")}','${t.replace(/'/g, "\\'")}');">${esc(t)}</span>`)
			.join("");
    ...
    <div class="gnav"><a onclick="app.nav.home()">דף הבית</a> › ${breadLinks} › <span>${esc(g.title)}</span></div>
    <h1>${esc(g.title)}</h1>
